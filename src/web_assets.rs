...
document.addEventListener('DOMContentLoaded', () => {
  ...
  const refreshFailuresButton =
    document.getElementById('refresh-failures');
  ...
  document.addEventListener(
    ...
    async (event) => {
      ...
      if (action === 'clear-failed') {
        ...
        confirmText =
          `Fehlerstatus für dieses Release ` +
          `zurücksetzen?\n\n${path}`;

        busyText = 'Wird zurückgesetzt …';

        successText =
          'Der Fehlerstatus wurde zurückgesetzt.';
      }
